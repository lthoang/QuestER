...
def qa_stats(data_dir):
    ...
    with open(os.path.join(data_dir, 'qa.txt'), 'r') as f:
        for line in f:
            ...
            tokens = line.split('\t\t')
            asin = tokens[0]
            for qa_text in tokens[1:]:
                n_questions += 1
                qa_toks = qa_text.split('\t')
                question = qa_toks[0]
                answers = qa_toks[2::2]
                n_answers += len(answers)
    print('# items =', n_items)
    print('# questions =', n_questions)
    print('# answers =', n_answers)
    print('# answers / # questions =', n_answers / n_questions)
...
def export_ranked_questions(model, path, max_num_question=32, max_num_answer=1):
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            for ranked_id in (-model.Gamma[iid][:min(len(item_qas_ids), max_num_question)]).argsort():
                ...
                if len(item_qa_ids) > 1:
                    answer_ids = item_qa_ids[1:1+max_num_answer]
                    question_answer_text = '\t'.join([question_answer_text] + [model.train_set.review_and_item_qa_text.qas[aid] for aid in answer_ids])
...
def export_ranked_questions_from_narreq(model, path, max_num_question=32):
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            item_qas_ids = model.train_set.review_and_item_qa_text.item_qas[iid]
            line = str(raw_iid)
            for ranked_id in (-model.A[iid, :min(len(item_qas_ids), max_num_question)]).argsort():
                item_qa_ids = item_qas_ids[ranked_id]
                qid = item_qa_ids[0]
                question_answer_text = model.train_set.review_and_item_qa_text.qas[qid]
                if len(item_qa_ids) > 1:
                    first_answer_idx = item_qa_ids[1]
                    question_answer_text = '{}\t{}'.format(question_answer_text, model.train_set.review_and_item_qa_text.qas[first_answer_idx])
                line = '{}\t\t{}'.format(line, question_answer_text)
            f.write('{}\n'.format(line))
...
def export_ranked_questions_from_narqre(model, path, max_num_question=32):
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            item_qas_ids = model.train_set.review_and_item_qa_text.item_qas[iid]
            line = str(raw_iid)
            for ranked_id in (-model.Aq[iid, :min(len(item_qas_ids), max_num_question)]).argsort():
                item_qa_ids = item_qas_ids[ranked_id]
                qid = item_qa_ids[0]
                question_answer_text = model.train_set.review_and_item_qa_text.qas[qid]
                if len(item_qa_ids) > 1:
...
def export_ranked_questions_from_hftq(model, path, max_num_question=32, max_num_answer=1):
    from collections import Counter
    theta = np.exp(model.model.kappa * model.model.gamma_i) / np.expand_dims(np.exp(model.model.kappa * model.model.gamma_i).sum(1), -1)
    ...
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            for qa in item_qas:
                ...
                freq_dist = freq_dist / freq_dist.sum() if freq_dist.sum() > 0 else freq_dist
            ...
            for ranked_id in ranked:
                ...
                if len(item_qa_ids) > 1:
                    first_answer_idx = item_qa_ids[1]
                    question_answer_text = '{}\t{}'.format(question_answer_text, model.train_set.qa_text.corpus[first_answer_idx])
                line = '{}\t\t{}'.format(line, question_answer_text)
            f.write('{}\n'.format(line))
...
def export_useful_review_ranking(model, path):
    ...
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            ranked = (-model.Beta[iid, top_question_id, :len(item_reviewer_ids)]).argsort()[:len(item_reviewer_ids)]
...
def export_important_question_ranking_from_hftq(model, path, max_num_question=32, max_num_answer=1):
    from collections import Counter
    theta = np.exp(model.model.kappa * model.model.gamma_i) / np.expand_dims(np.exp(model.model.kappa * model.model.gamma_i).sum(1), -1)
    topic_assignment = model.model.topic_assignment
    with open(path, 'w') as f:
...
def export_most_useful_review(model, path):
    ...
    with open(path, 'w') as f:
        ...
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            reviewer_id, review_id = item_review_ids[model.Beta[iid, top_question_id, :len(item_review_ids)].argmax()]
...
def export_useful_review_ranking_from_narre(model, path):
    ...
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            item_reviewer_ids = list(model.train_set.review_text.item_review[iid].keys())
            ranked = (-model.A[iid, :len(item_reviewer_ids)]).argsort()[:len(item_reviewer_ids)]
            f.write('{},{}\n'.format(raw_iid, ','.join([str(uid2rawuid[item_reviewer_ids[i]]) for i in ranked])))
...
def export_useful_review_ranking_from_hft(model, path, max_num_review=32):
    ...
    with open(path, 'w') as f:
        for raw_iid, iid in model.train_set.iid_map.items():
            item_reviewer_ids = list(model.train_set.review_text.item_review[iid].keys())[:max_num_review]
            item_review_ids = list(model.train_set.review_text.item_review[iid].values())[:max_num_review]
            item_reviews = [
                model.train_set.item_text.vocab.to_idx(tokens)
                for tokens in model.train_set.item_text.tokenizer.batch_tokenize(
                    [model.train_set.review_text.corpus[review_id] for review_id in item_review_ids]
                )
            ]
            item_document = model.documents[iid]
            scores = []
...
def export_most_useful_review_from_hft(model, path, max_num_review=32):
    ...
    with open(path, 'w') as f:
        ...
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            f.write('{}\t{}\t{}\n'.format(raw_iid, str(uid2rawuid[item_reviewer_ids[top_id]]), model.train_set.review_text.corpus[item_review_ids[top_id]]))
...
def export_most_useful_review_from_narre(model, path):
    ...
    with open(path, 'w') as f:
        ...
        for raw_iid, iid in model.train_set.iid_map.items():
            ...
            f.write('{}\t{}\t{}\n'.format(raw_iid, str(uid2rawuid[reviewer_id]), model.train_set.review_text.corpus[review_id]))
...
def export_erqa_explanations(selected_model, explanation_dir):
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        item_question_ids = sum(selected_model.train_set.qa_text.item_qas[mapped_iid], [])
        item_questions = [selected_model.train_set.qa_text.corpus[idx] for idx in item_question_ids]
        ...
        review_attention_weights = selected_model.Beta[mapped_iid, :n_questions, :n_reviews]
        with open(os.path.join(explanation_dir, '{}.txt'.format(raw_iid)), 'w') as f:
            ...
            f.write('\n')
            f.write('Question attention weights:\n{}\n'.format(','.join([str(weight) for weight in question_attention_weights])))
            f.write('Review attention weights:\n{}\n'.format(
                '\n'.join([
                    ','.join([str(review_attention_weights[qidx, ridx]) for ridx in range(n_reviews)])
...
def export_erqa_sentence_level_explanations(selected_model, explanation_dir, top_k_question=1, top_k_sentence=1):
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        item_question_ids = sum(selected_model.train_set.review_sentence_and_item_qa_text.item_qas[mapped_iid], [])
        item_questions = [selected_model.train_set.review_sentence_and_item_qa_text.corpus[idx] for idx in item_question_ids]

        sentence_ids = []
        for inc, (jdx, j_sentence_ids) in enumerate(selected_model.train_set.review_sentence_and_item_qa_text.item_review_sentence[mapped_iid].items()):
            if selected_model.max_num_review is not None and inc == selected_model.max_num_review:
                break
            for s_inc, sentence_idx in enumerate(j_sentence_ids):
                if selected_model.max_num_sentence is not None and s_inc == selected_model.max_num_sentence:
                    break
                sentence_ids.append(sentence_idx)

        item_sentences = [selected_model.train_set.review_sentence_and_item_qa_text.corpus[sentence_idx] for sentence_idx in sentence_ids]
        # item_reviews = [selected_model.train_set.review_text.corpus[review_idx] for review_idx in selected_model.train_set.review_text.item_review[mapped_iid].values()]

        n_questions = len(item_questions)
        n_sentences = len(item_sentences)
        top_question_ids = selected_model.Gamma[mapped_iid].argsort()[::-1][:min(top_k_question, n_questions)]
        top_sentence_ids = selected_model.Beta[mapped_iid, top_question_ids[0]].argsort()[::-1][:min(top_k_sentence, n_sentences)]
        question_attention_weights = selected_model.Gamma[mapped_iid, :n_questions]
        sentence_attention_weights = selected_model.Beta[mapped_iid, :n_questions, :n_sentences]
        with open(os.path.join(explanation_dir, '{}.txt'.format(raw_iid)), 'w') as f:
            f.write('Top question(s):\n{}\n'.format('\n'.join([item_questions[top_question_idx] for top_question_idx in top_question_ids])))
            f.write('Top sentence(s):\n{}\n'.format('\n'.join([item_sentences[top_sentence_idx] for top_sentence_idx in top_sentence_ids])))
            f.write('\n')
            f.write('Question attention weights:\n{}\n'.format(','.join([str(weight) for weight in question_attention_weights])))
            f.write('Sentence attention weights:\n{}\n'.format(
                '\n'.join([
                    ','.join([str(sentence_attention_weights[qidx, sidx]) for sidx in range(n_sentences)])
                    for qidx in range(n_questions)
                ])
            ))
            f.write('Questions:\n')
...
def export_erqa_item_explanations(selected_model, output_dir, max_num_question=32, max_num_review=32, max_num_answer=1):
    import json
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        ...
        for inc, item_question_answers_ids in enumerate(selected_model.train_set.review_and_item_qa_text.item_qas[mapped_iid]):
            if max_num_question is not None and inc == max_num_question:
                break
            question = {
                'questionText': selected_model.train_set.review_and_item_qa_text.qas[item_question_answers_ids[0]],
                'answerText': ''
            }
            if len(item_question_answers_ids) > 1:
                answers = '\t'.join([selected_model.train_set.review_and_item_qa_text.qas[aid] for aid in item_question_answers_ids[1:1+max_num_answer]])
        ...
        with open(os.path.join(saved_dir, '{}.json'.format(raw_iid)), 'w') as f:
            data = {
                ...
                "Beta": selected_model.Beta[mapped_iid, :n_question, :n_review].tolist(),
            }
            json.dump(data, f)

def export_item_explanations_from_narqre(selected_model, output_dir, max_num_question=32, max_num_review=32):
    import json
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        questions = []
        for inc, item_question_answers_ids in enumerate(selected_model.train_set.review_and_item_qa_text.item_qas[mapped_iid]):
            ...
            if len(item_question_answers_ids) > 1:
                firstAnswer = selected_model.train_set.review_and_item_qa_text.qas[item_question_answers_ids[1]]
...
def export_narre_item_explanations(selected_model, output_dir, max_num_review=32):
    import json
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        reviews = [selected_model.train_set.review_text.corpus[rid] for _, rid in list(selected_model.train_set.review_text.item_review[mapped_iid].items())]
        n_review = min(len(reviews), max_num_review)
        reviews = reviews[:n_review]
        saved_dir = os.path.join(output_dir, '/'.join(str(raw_iid)))
        os.makedirs(saved_dir, exist_ok=True)
...
def export_narreq_item_explanations(selected_model, output_dir, max_num_review=32):
    import json
    for raw_iid, mapped_iid in selected_model.train_set.iid_map.items():
        reviews = [selected_model.train_set.review_text.corpus[rid] for _, rid in list(selected_model.train_set.review_text.item_review[mapped_iid].items())]
        n_review = min(len(reviews), max_num_review)
